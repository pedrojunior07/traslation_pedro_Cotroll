...
class TranslatorUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        # Inicializar novos clientes
        self.config = ConfigManager()
        self.libre_client = None
    ...
    def _init_clients(self) -> None:
        """Inicializa clientes diretos (LibreTranslate, Claude, Database)"""
        try:
            ...
            print(f"Aviso: Erro ao inicializar LibreTranslate client: {e}")

        try:
            # Claude client (só inicializa se tiver API key)
            ...
            print(f"Aviso: Erro ao inicializar Claude client: {e}")

        try:
            # Database
            self.db = Database(self.config)
        except Exception as e:
            print(f"Aviso: Erro ao conectar ao MySQL: {e}")
    ...
    def _build_layout(self) -> None:
        ...
        # Botão Definições no canto direito
        settings_btn = ttk.Button(header, text="⚙ Definições", command=self.open_settings)
    ...
    def open_settings(self) -> None:
        ...
        try:
            ...
            # Obter device_id se disponível
            device_id = None
            if self.db:
                try:
                    # Tentar obter device_id do token atual ou config
                    device_id = 1  # Default, pode melhorar obtendo do backend
                except:
                    pass

            # Abrir janela
            SettingsWindow(self.root, device_id=device_id)

            # Recarregar clientes após fechar a janela de definições
            # (caso o usuário tenha mudado a API key ou outras configurações)
            self.root.after(500, self._reinit_clients_if_needed)

        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao abrir definições:\n{str(e)}")

    def _reinit_clients_if_needed(self) -> None:
        """Reinicializa clientes se configuração mudou"""
        try:
            # Recarregar config
            old_api_key = self.config.get("claude_api_key", "")
            self.config = ConfigManager()  # Reload
            new_api_key = self.config.get("claude_api_key", "")

            # Se API key mudou, reinicializar Claude client
            if old_api_key != new_api_key:
                if new_api_key:
                    claude_model = self.config.get("claude_model", "claude-3-5-sonnet-20241022")
                    self.claude_client = ClaudeClient(api_key=new_api_key, model=claude_model)
                    print("✓ Claude client atualizado")
                else:
                    self.claude_client = None

            # Reinicializar LibreTranslate client com nova config
            libre_url = self.config.get("libretranslate_url", "http://102.211.186.44/translate")
            libre_timeout = self.config.get("libretranslate_timeout", 15.0)
            self.libre_client = LibreTranslateClient(base_url=libre_url, timeout=libre_timeout)

        except Exception as e:
            print(f"Aviso: Erro ao reinicializar clientes: {e}")
